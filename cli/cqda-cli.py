...
from cqda.models import KGReasoning
from cqda.cqd import CQD
from cqda.cqd import Norms
from cqda.cqd import Negations

from cqda.dataloader import TestDataset, TrainDataset, SingledirectionalOneShotIterator
from cqda.cqd import CQDTrainDataset
...
from cqda.util import flatten_query, parse_time, set_global_seed, eval_tuple, get_query_name_dict
...
def parse_args(args=None):
    ...
    parser.add_argument('--aim_servr', default=None, type=str, help='Static ip:port for aim remote tracking server')
    parser.add_argument('--no-aim', action='store_true', default=False)
    ...
    parser.add_argument('--geo', default='vec', type=str, choices=['vec', 'box', 'beta', 'cqd'],
                        help='the reasoning model, vec for GQE, box for Query2box, beta for BetaE, cqd for CQD')
    parser.add_argument('--print_on_screen', action='store_true')

    parser.add_argument('--reg_weight', default=1e-3, type=float)
    parser.add_argument('--optimizer', choices=['sgd', 'adamw', 'adam', 'adagrad'], default='adam')
    parser.add_argument('--cqd-type', '--cqd', default='discrete', type=str, choices=['continuous', 'discrete'])
    parser.add_argument('--cqd-t-norm', default=Norms.NORM_FAMILIES[1], type=str, choices=Norms.NORM_FAMILIES)
    parser.add_argument('--cqd-negation', default=Negations.STANDARD_NEGATION, type=str, choices=Negations.NEGATION_FAMILIES)

    parser.add_argument('--cqd-k', default=5, type=int)
    parser.add_argument('--cqd-test-k', default=None, type=int)

    parser.add_argument('--cqd-sigmoid-normalize-scores', '--cqd-sigmoid-normalize', action='store_true', default=False)
    parser.add_argument('--cqd-minmax-normalize-scores', '--cqd-minmax-normalize', action='store_true', default=False)
    parser.add_argument('--cqd-softmax-normalize-scores', '--cqd-softmax-normalize', action='store_true', default=False)
    parser.add_argument('--use-qa-iterator', action='store_true', default=False)

    # XXX - split tasks at train and eval
    ...
    parser.add_argument('--eval-tasks', default=None, type=str)
    ...
    parser.add_argument('--disable-warmup', action='store_true', default=False)
    parser.add_argument('--disable-saving', action='store_true', default=False)

    parser.add_argument('--loss', choices=['nlp', 'nll',
                                           'mr', 'amr', 'mr-sum', 'amr-sum',
                                           'ce', 'bce', 'abce', 'bcel', 'abcel',
                                           'hinge', 'q2b', '1vsall'], default='1vsall')
    ...
    parser.add_argument('--use-per-relation-params', action='store_true', default=False)

    parser.add_argument('--score-transform', default="none", type=str,
                        choices=['none', 'affine', 'deep'])
    parser.add_argument('--score-transform-input', default=None, type=str)
    parser.add_argument('--score-transform-hidden', default=None, type=int)

    parser.add_argument('--gradient-accumulation-steps', default=1, type=int)
    parser.add_argument('--init-size', default=1e-3, type=float)

    parser.add_argument('--projection-type', choices=['replace', 'concat'], default='replace')
    parser.add_argument('--project-entity-emb', default=None, type=int)
    parser.add_argument('--project-predicate-emb', default=None, type=int)

    res = parser.parse_args(args)

    if res.cuda is True and torch.cuda.is_available() is False:
...
def save_model(model, optimizer, save_variable_list, args):
    ...
    if 'aim_run' in argparse_dict:
        argparse_dict = {k: v for k, v in argparse_dict.items() if k != 'aim_run'}
...
def load_data(args, tasks, eval_tasks = None):
    '''
    ...
    for name in all_tasks:
        ...
        if name not in eval_tasks or evaluate_union != args.evaluate_union:
            query_structure = name_query_dict[name if 'u' not in name else '-'.join([name, evaluate_union])]
...
def main(args):
    ...
    aim_run = None
    if args.no_aim is False:
        if args.aim_servr is not None:
        ...
        aim_run["params"] = vars(args)

    args.aim_run = aim_run

    set_global_seed(args.seed)
    tasks = args.tasks.split('.')
    eval_tasks = args.eval_tasks.split('.') if args.eval_tasks is not None else None
    ...
    if args.save_path is None and not args.disable_saving:
    ...
    if not args.disable_saving and not os.path.exists(args.save_path):
    ...
    test_queries, test_hard_answers, test_easy_answers = load_data(args, tasks, eval_tasks=eval_tasks)
    ...
    if args.geo == 'cqd':
        model = CQD(nentity,
                    ...
                    k=args.cqd_k,
                    test_k=args.cqd_test_k,
                    do_sigmoid_normalize=args.cqd_sigmoid_normalize_scores,
                    do_minmax_normalize=args.cqd_minmax_normalize_scores,
                    do_softmax_normalize=args.cqd_softmax_normalize_scores,
                    use_cuda=args.cuda,
                    freeze_embeddings=args.freeze_embeddings,
                    use_logits=args.use_logits,
                    use_per_relation_params=args.use_per_relation_params,
                    aim_run=aim_run,
                    score_transform=args.score_transform,
                    score_transform_input=args.score_transform_input,
                    score_transform_hidden=args.score_transform_hidden,
                    init_size=args.init_size,
                    projection_type=args.projection_type,
                    project_entity_emb=args.project_entity_emb,
                    project_predicate_emb=args.project_predicate_emb)
    else:
        model = KGReasoning(
            nentity=nentity,
            nrelation=nrelation,
    ...
    if args.do_train:
        ...
        trainable_params = [p for p in filter(lambda p: p.requires_grad, model.parameters())]

        if len(trainable_params) == 0:
            print('No trainable parameters found -- disabling training')
            args.do_train = False
        else:
    ...
    if args.checkpoint_path is not None:
        ...
        if args.do_train and not args.new_optimizer:
    ...
    if args.do_train:
        ...
        logging.info(f'learning_rate = {current_learning_rate:.8f}')
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            # print('Weights', model.score_transform_fun.weight)
            # print('Bias', model.score_transform_fun.bias)
